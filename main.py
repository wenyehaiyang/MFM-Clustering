...
def mfm_xnn():
    ...
    data = np.genfromtxt('OPEs2.csv', delimiter=',', names=True)
    data1 = np.array([[j for j in list(i)[1:]] for i in data])
    row, col = data1.shape
    for r in range(row):
        for c in range(col):
...
def mfm_xnn_test():
    ...
    k_SSE(pd.DataFrame(data1), 15, ax3, title='(a)', ok_k=4)
    data = np.genfromtxt('采样点m.csv', delimiter=',', names=True)
    data1 = np.array([[j for j in list(i)[1:]] for i in data])
    row, col = data1.shape
    for r in range(row):
    ...
    # test_modal_weight(data1, ax4, test_limit=(1e-4, 1e2), method='optics', min_samples=2, plot_title='(d)')
    k_silhouette(pd.DataFrame(data1), 15, ax4, title='(b)', ok_k=4)

    plt.subplots_adjust(left=0.083, right=0.98, bottom=0.13, top=0.93, wspace=0.225, hspace=0.33)
    plt.savefig("output_t1.png", dpi=300)
    plt.show()
